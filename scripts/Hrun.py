import sys
import gcepy.highdim_model as hm
...
def ptform(x, dim):
    return x*(hm.pmax[:dim]-hm.pmin[:dim])+hm.pmin[:dim]
...
sampler_dn = dn.NestedSampler(lambda x: hm.jjlnlike(x, EBIN, ex_num=EXNUM), lambda x: jptform(x, PDIM), PDIM)

sampler_dn.run_nested(dlogz=1)

sampler_nr = nr.infer.MCMC(nr.infer.NUTS(potential_fn = lambda t: -hm.jjlnprob(t, bin_no=EBIN, ex_num=EXNUM)), num_samples=10000, num_warmup=100, jit_model_args=True, chain_method='vectorized')
...
lls_nr = jnp.array([hm.jjlnprob(x, bin_no=EBIN, ex_num=EXNUM) for x in samples_nr])
