...
def ptform(x, dim):
    return x*(gp._lm.pmax[:dim]-gp._lm.pmin[:dim])+gp._lm.pmin[:dim]
...
sampler_dn = dn.NestedSampler(lambda x: gp.lnlike('low', x, EBIN, ex_num=EXNUM), lambda x: jptform(x, PDIM), PDIM)

sampler_dn.run_nested(dlogz=1)

sampler_nr = nr.infer.MCMC(nr.infer.NUTS(potential_fn = lambda t: -gp.lnlike('low', t, bin_no=EBIN, ex_num=EXNUM)), num_samples=10000, num_warmup=100, jit_model_args=True, chain_method='vectorized')
